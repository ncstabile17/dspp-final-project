...
TODO: Add additional visualizations for evaluating the predicted values
TODO: Add tables describing the error terms (differentiate between MAE and RMSE; show measures of central tendency; how RMSE compares to the range, quartiles, and standard deviation; show the range and quartiles of the error terms)
TODO: Think about taking out the really large outlier(s) and redoing the models
TODO: Add additional summary stats (either right at the beginning or before the models, maybe some of both)
...
predict_investment_recipe <- 
 ...
 # transforming the outcome variable was causing problems with the predictions and couldn't figure it out in time
...
predict_investment_rf_best_predictions <- 
  ...
  filter(total_proj_investment < 200000000)
...
predict_investment_rf_best_predictions %>% 
  ggplot() +
  geom_point(
    aes(x = predicted_proj_investment, y = total_proj_investment),
    alpha = .5) +
  scale_x_continuous(labels = comma) + 
...
summary(predict_investment_data$log_total_proj_investment)

# TODO: Add some sort of graph comparing predictions and estimate and undo log transform
```
Analysis: The model is much better at predicting small investments, which is what we would expect since the data is very right skewed and the vast majority of the investments were small numbers (talk about median, quartiles and things like that). Maybe add some more descriptive data at the top before other graphs (should use the summary function). 
...
#  filter(total_proj_affordable < 500)
