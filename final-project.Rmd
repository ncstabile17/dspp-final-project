...
#TODO: recode the different AMI variables so that they line up and there is one set (if necessary, maybe I don't care as much?)
...
combined_data <- 
  ...
  mutate(total_dmped_investment = replace_na(total_dmped_investment, 0),
         ...
         total_proj_units = if_else(total_dmped_affordable > total_units, total_dmped_affordable, total_units))
...
comparing_units <- combined_data %>% 
  mutate(total_dmped_affordable = replace_na(total_dmped_affordable, 0),
         ...
         total_units = replace_na(total_units, 0)) %>% 
  select(investment_address, total_dmped_affordable, total_dhcd_affordable, unit_diff, more_units, total_units, total_proj_units, market_rate_units, total_proj_affordable)
