...
# Doing similar importing and cleaning for additional data that has market rate units
affordable_proj_market_rate <- read_csv("data/affordable-projects-market-rate.csv") %>% 
  ...
  mutate(
    ...
    invest_amount = as.numeric(gsub('[\\$,]', '', invest_amount)),
    lihtc_amount = as.numeric(gsub('[\\$,]', '', lihtc_amount))
  ) %>% 
