...
affordable_proj_market_rate <- read_csv("data/affordable-projects-market-rate.csv") %>% 
  ...
  rename(
    ...
    market_rate_ami_81 = `81%+ AMI / Market Rate`,
...
investment_by_project <- affordable_proj_market_rate %>% 
  ...
  summarize(total_loan_grant_amount = sum(loan_grant_amount))
...
# Merging the existing data set with market rate data set

combined_data <- 
  full_join(dc_housing_investment, affordable_proj_market_rate, by = "investment_address")

test_columns <- combined_data %>% 
  filter(!is.na(total_affordable)) %>% 
  mutate(same_30 = if_else(ami_30 == ami_0_30, TRUE, FALSE),
         same_50 = if_else(ami_50 == ami_31_50, TRUE, FALSE),
         same_affordable = if_else(total_affordable == affordable_units, TRUE, FALSE))
