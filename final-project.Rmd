...
---
**Data Sources:** [DC FY19 Economic Development Return on Investment Data spreadsheet](https://dmped.dc.gov/sites/default/files/dc/sites/dmped/publication/attachments/Return%20on%20Investment%20Data_FY15-19_v3.xlsx)
...
TODO: Mention taking out the really large outlier(s) and redoing the models
...
total_unit_investment_data %>% 
  ...
  geom_smooth(method = lm, se = FALSE) +
  stat_cor(aes(label = ..rr.label..), geom = "label") +
  labs(title = "No relationship between investment efficiency and deeply affordable units in project",
       color = "Project with Market Rate Units") +
  scale_x_continuous(labels = dollar_format()) +
  xlab('Government Investment Per Unit Produced') +
...
total_unit_investment_data %>% 
  ...
  geom_smooth(method = lm, se = FALSE) +
  stat_cor(aes(label = ..rr.label..), geom = "label") +
  labs(title = "No relationship between investment efficiency and moderate income units in project",
       color = "Project with Market Rate Units") +
  scale_x_continuous(labels = dollar_format()) +
  xlab('Government Investment Per Unit Produced') +
...
dc_aff_housing_merged_agg %>% 
  ...
  ggtitle("Affordable housing production concentrated east of Anacostia River and in center of city") +
  theme_void()
...
dc_aff_housing_merged_agg %>% 
  ...
  ggtitle("Deeply affordable housing production even more concentrated in particular Census tracts") +
  theme_void()
...
summary(predict_investment_data$total_proj_investment)
...
summary(predict_aff_units_data$total_proj_affordable)
