...
prediction_data <- combined_data %>% 
  select(ward, dc_residents_employed, num_cbes, total_bedrooms, ami_0_30, ami_31_50, ami_51_80, ami_81, market_rate_units, total_proj_investment, proj_lihtc_amount, total_proj_affordable, total_funding_sources, investment_year, quadrant) %>% 
...
predict_investment_recipe <- recipe(total_proj_investment ~ ., data = predict_investment_train) %>%
  ...
  step_other(investment_year, threshold = 0.01, 
             other = "other") %>%
  step_other(total_proj_funding_sources, threshold = 0.1, 
             other = "OTHER")
